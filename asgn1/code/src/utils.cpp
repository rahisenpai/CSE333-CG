...
extern int selectedPhantomPoint;
...
std::vector<float> rawPhantomPoints;
...
//adds phantom points to the rawPhantomPoints array to capture events
void addPhantomPoints(std::vector<float> &points, int w, int h) {
    rawPhantomPoints.clear();
    int npts = points.size()/3;
    for(int i=0; i<npts; i++) {
        float rescaled_x = points[3*i];
        float rescaled_y = points[3*i+1];
        float x = w * ((rescaled_x + 1.0) / 2.0);
        float y = h - h * ((rescaled_y + 1.0) / 2.0);
        rawPhantomPoints.push_back(x);
        rawPhantomPoints.push_back(y);
    }
}
...
// Search nearest phantom point to (x, y) and set its index to
// selectedPhantomPoint (return true), else -1 (return false)
bool searchNearestPhantomPoint(float x, float y) {
  size_t npts = rawPhantomPoints.size()/2;
  if(npts > 0) {
	  float _x, _y, dist2 = 0.0f;
		float thresh2 = selectionThreshold*selectionThreshold;
	  for(size_t i=0; i<npts; i++) {
			_x = rawPhantomPoints[2*i];
			_y = rawPhantomPoints[2*i+1];
			dist2 = (x-_x)*(x-_x) + (y-_y)*(y-_y);
			if(dist2 <= thresh2) {
			  selectedPhantomPoint = i;
				return 1;
			}
		}
	}

	selectedPhantomPoint = -1;
	return 0;
}

//catches the changes made to phantom points
void editPhantomPoint(std::vector<float> &points, float x, float y, int w, int h){
    if(selectedPhantomPoint < 0) return;
		if(selectedPhantomPoint >= points.size()/3) return;

    float rescaled_x = -1.0 + ((1.0*x - 0) / (w - 0)) * (1.0 - (-1.0));
    float rescaled_y = -1.0 + ((1.0*(h - y) - 0) / (h - 0)) * (1.0 - (-1.0));
    points[selectedPhantomPoint*3] = rescaled_x;
    points[selectedPhantomPoint*3+1] = rescaled_y;
    points[selectedPhantomPoint*3+2] = 0.0; // Z-coordinate

		rawPhantomPoints[selectedPhantomPoint*2] = x;
		rawPhantomPoints[selectedPhantomPoint*2+1] = y;
}
