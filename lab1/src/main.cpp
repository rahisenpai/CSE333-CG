...
void createOctagonObject(unsigned int &, unsigned int &); // The function you must edit
...
int main()
{
    ...
    createOctagonObject(shaderProgram, VAO);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glDrawArrays(GL_TRIANGLES, 0, 8*3); // Edit This, specify the number of vertices in the last argument.
        glUniform3f(vColor_uniform, 0.0, 0.0, 0.0);
        glDrawArrays(GL_LINE_STRIP, 0, 8*3); // Edit This, specify the number of vertices in the last argument.
...
void createOctagonObject(unsigned int &program, unsigned int &octa_VAO)
{
    ...
    // --------------------- Edit This ---------------------
    // Vertex Data
    // GLfloat triangle_vertices[] = {0, 0, 0,
    //                             10, 0, 0,
    //                             10, 20, 0,
    //                             0, 20, 0};
    GLfloat triangle_vertices[] = {0, 0, 0,
    ...
    // Indice Order
    // GLushort triangle_indices[] = {3, 0, 1, 2, 3, 1};
    GLushort triangle_indices[] = {0,1,2, 0,2,3, 0,3,4, 0,4,5, 0,5,6, 0,6,7, 0,7,8, 0,8,1};

    int nVertices = sizeof(triangle_indices) / sizeof(triangle_indices[0]);

    GLfloat *expanded_vertices = new GLfloat[nVertices*3];
    for(int i=0; i<nVertices; i++) {
        expanded_vertices[i*3] = triangle_vertices[triangle_indices[i]*3];
        expanded_vertices[i*3 + 1] = triangle_vertices[triangle_indices[i]*3+1];
        expanded_vertices[i*3 + 2] = triangle_vertices[triangle_indices[i]*3+2];
    }
    // -----------------------------------------------------

    // Generate VAO object
    glGenVertexArrays(1, &octa_VAO);
    glBindVertexArray(octa_VAO);
